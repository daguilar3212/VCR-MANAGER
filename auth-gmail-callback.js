...
// Parse XML server-side using regex (no DOMParser on server)
function parseXMLServer(xmlStr) {
  ...
  return {
    ...
    last_four: consecutivo.slice(-4),
    emission_date: fechaEmision,
    supplier_name: supName,
    supplier_commercial_name: supComm,
    supplier_id: supId,
    supplier_id_type: supIdType,
    supplier_email: supEmail,
    supplier_phone: supPhone,
    currency, exchange_rate: exchangeRate,
    subtotal, discount_total: discountTotal, tax_total: taxTotal,
    other_charges: otherCharges, other_charges_detail: otherChargesDetail,
    total,
    payment_method_code: payCode,
    payment_method_label: payMap[payCode] || payCode,
    is_credit_card: payCode === "02",
    credit_days: creditDays,
    detected_plate: detectedPlate,
    category_id: catId,
    group_id: groupMap[catId] || "otros_gastos",
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get last sync date, default to today
    ...
    // Search for emails with attachments after last sync
    ...
    for (const msg of searchResult.messages) {
      ...
      for (const part of parts) {
        if (part.filename?.toLowerCase().endsWith('.xml')) xmlPart = part;
        if (part.filename?.toLowerCase().endsWith('.pdf')) hasPdf = true;
        // Check nested parts
        if (part.parts) {
          for (const sub of part.parts) {
            if (sub.filename?.toLowerCase().endsWith('.xml')) xmlPart = sub;
            if (sub.filename?.toLowerCase().endsWith('.pdf')) hasPdf = sub;
          }
        }
      }

      // Only process if has both XML and PDF
      if (!xmlPart || !hasPdf) { skipped++; continue; }

      // Download XML attachment
      ...
      // Check if this invoice already exists by xml_key
      const { data: existingInv } = await supabase.from('invoices').select('id').eq('xml_key', parsed.xml_key).limit(1);
      if (existingInv && existingInv.length > 0) { skipped++; continue; }

      // Try to match plate against vehicles
      ...
      const { data: inv, error: invError } = await supabase.from('invoices').insert({
        ...
        supplier_id_type: parsed.supplier_id_type,
        ...
        other_charges_detail: parsed.other_charges_detail,
        total: parsed.total,
      ...
      if (invError) { console.error('Insert error:', invError); continue; }
    ...
    return res.json({ processed, skipped, total: searchResult.messages.length });
  } catch (err) {
